...
class FrontendRouteTester {
  ...
  async testContentRoutes() {
    ...
    for (const route of contentRoutes) {
      await this.runTest(`GET ${route}`, this.testBasicRoute.bind(this), route);
    }
  }
  ...
  async testErrorHandling() {
    ...
    const errorRoutes = ['/invalid-route', '/nonexistent-page', '/s/invalid-page'];

    for (const route of errorRoutes) {
      await this.runTest(`Error handling for ${route}`, this.testErrorRoute.bind(this), route);
    }
  }
  ...
  async testSEOElements() {
    ...
    await this.runTest('SEO meta tags test', this.testSEOMetaTags.bind(this));

    await this.runTest('Canonical URLs test', this.testCanonicalURLs.bind(this));
  }

  async testResponsiveness() {
    console.log('📱 Testing responsiveness...');

    await this.runTest('Mobile viewport test', this.testMobileViewport.bind(this));

    await this.runTest('Desktop viewport test', this.testDesktopViewport.bind(this));
  }
  ...
  async testSpecificContentRoute(route) {
    try {
      ...
      const hasContentStructure = await this.page.$(
        '.content-page, .content-header, [data-content]'
      );
  ...
  async testLegacyRedirect(fromRoute, toRoute) {
    try {
      ...
      if (currentUrl === expectedUrl) {
        ...
        return {
          status: 'failed',
          message: `Expected redirect to ${expectedUrl}, got ${currentUrl}`,
        };
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.status() === 404) {
        ...
        if (has404Content) {
          ...
          return {
            status: 'warning',
            message: '404 status returned but no clear 404 page content',
          };
        }
      } else {
  ...
  async testCanonicalURLs() {
    try {
      ...
      const canonicalUrl = await this.page
