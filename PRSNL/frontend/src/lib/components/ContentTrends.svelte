<script lang="ts">
  ...
  function generateDNAHelix() {
    ...
    significantData.forEach((point, timeIndex) => {
      const heightPosition =
        (timeIndex / (significantData.length - 1)) * helixPitch * 2 - helixPitch;
      ...
      contentTypes.forEach((type, typeIndex) => {
        ...
        for (let strandIndex = 0; strandIndex < 2; strandIndex++) {
          const baseAngle =
            (timeIndex / significantData.length) * Math.PI * 8 + strandIndex * Math.PI;
  ...
  function getSignificantDNAData(dataPoints: DataPoint[]) {
    const totalItems = dataPoints.reduce(
      (sum, point) =>
        sum +
        (point.articles || 0) +
        (point.videos || 0) +
  ...
  function drawDNAConnections() {
    dnaConnections.forEach((connection) => {
      ...
      const color1 = connection.strand1.color.startsWith('#')
        ? connection.strand1.color + '80'
        : 'rgba(255,255,255,0.5)';
      const color2 = connection.color.startsWith('#')
        ? connection.color + '60'
        : 'rgba(255,255,255,0.4)';
      const color3 = connection.strand2.color.startsWith('#')
        ? connection.strand2.color + '80'
        : 'rgba(255,255,255,0.5)';
...
<div class="dna-helix" bind:this={container}>
  {#if data && data.length > 0}
    ...
    {#if hoveredStrand}
      <div
        class="dna-tooltip"
        style="left: {project3D(hoveredStrand.x, hoveredStrand.y, hoveredStrand.z).x +
          20}px; top: {project3D(hoveredStrand.x, hoveredStrand.y, hoveredStrand.z).y - 10}px;"
      >
