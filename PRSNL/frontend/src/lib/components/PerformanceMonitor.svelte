<script lang="ts" type="module">
  ...
  function updateMetrics() {
    ...
    if (videoPerformanceMetrics.memoryUsage.length > 0) {
      memoryUsage =
        videoPerformanceMetrics.memoryUsage[videoPerformanceMetrics.memoryUsage.length - 1].value;
    }
        ...
        <div class="metric-item">
          ...
          <div class="metric-value">
            {#if videoPerformanceMetrics.loadTimes.length > 0}
              {formatTime(
                videoPerformanceMetrics.loadTimes.reduce((sum, item) => sum + item.value, 0) /
                  videoPerformanceMetrics.loadTimes.length
              )}
            {:else}
              N/A
            {/if}
      ...
      <div class="settings-section">
        ...
        <label class="setting-toggle">
          <input
            ...
            on:change={() =>
              ($mediaSettings.enableProgressiveLoading = !$mediaSettings.enableProgressiveLoading)}
          />
          Progressive Loading
        </label>
      </div>
