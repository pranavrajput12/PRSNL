<script lang="ts">
  ...
  function createSkillTree() {
    const skills = [
      { id: 'speed-reader', name: 'Speed Reader', icon: '📚', description: 'Read articles 2x faster' },
      { id: 'video-binger', name: 'Video Binger', icon: '🎬', description: 'Process videos efficiently' },
      ...
      { id: 'memory-palace', name: 'Memory Palace', icon: '🏛️', description: 'Perfect recall ability' },
      { id: 'pattern-seeker', name: 'Pattern Seeker', icon: '🔍', description: 'Find hidden connections' }
    ];
  ...
  function drawSkillNodes() {
    skillNodes.forEach(node => {
      ...
      ctx.fillStyle = node.unlocked ? 
        (node.level > 0 ? avatar.auraColor : '#333333') : 
        '#1a1a1a';
      ctx.strokeStyle = isHovered ? '#ffffff' : 
        (node.unlocked ? avatar.auraColor : '#333333');
...
<div class="avatar-container" bind:this={container}>
  ...
  <div class="controls">
    <button on:click={() => experience.update(exp => exp + 250)}>
      Gain Experience
    </button>
    <button on:click={() => {
