<script lang="ts">
  ...
  function calculateBiodiversity() {
    ...
    const typeBalance =
      1 - Math.abs(0.25 - creatures.filter((c) => c.type === 'article').length / totalCreatures);
  ...
  function updateCreatures() {
    creatures.forEach((creature) => {
      ...
      switch (creature.behavior) {
        ...
        case 'feed':
          if (!creature.target) {
            const nearestPlant = plants.reduce((nearest, plant) => {
              ...
              return dist < Math.hypot(nearest.x - creature.x, nearest.y - creature.y)
                ? plant
                : nearest;
            });
        ...
        case 'socialize':
          const nearbyCreature = creatures.find(
            (c) =>
              c !== creature &&
  ...
  function drawBiodiversityMeter() {
    ...
    const scoreColor =
      $biodiversityScore > 70 ? '#00ff64' : $biodiversityScore > 40 ? '#ffa500' : '#ff0000';
...
<div class="terrarium-container" bind:this={container}>
  ...
  <div class="controls">
    <button class="season-btn" on:click={changeSeason}> Change Season </button>
