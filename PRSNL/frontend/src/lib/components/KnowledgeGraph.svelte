<script lang="ts">
  ...
  function setupVisualization() {
    ...
    // Add zoom behavior
    zoomHandler = d3.zoom<SVGSVGElement, unknown>()
      .scaleExtent([0.1, 4])
    ...
    nodes.filter(d => (d.importance || 0) > 0.7 || d.type === 'topic')
      .append("text")
    ...
    simulation = d3.forceSimulation<KnowledgeGraphNode>(data.nodes)
      .force("link", d3.forceLink<KnowledgeGraphNode, KnowledgeGraphLink>(data.links).id(d => d.id).distance(100))
      ...
      .force("collision", d3.forceCollide<KnowledgeGraphNode>().radius(d => Number(size(d.importance || 1)) + 15))
      .on("tick", ticked);
    ...
    // Drag functions
    function dragstarted(event: d3.D3DragEvent<SVGGElement, KnowledgeGraphNode, KnowledgeGraphNode>) {
  ...
  function highlightConnections(nodeId: string) {
    ...
    d3.select(svg).select(`.node-${nodeId}`)
      .style("opacity", "1");
