...
>
  ...
  <div class="processing-effects {isHovered ? 'active' : ''}">
    <div class="compute-rays">
      {#each Array(8) as _, i}
        <div class="compute-ray" style="transform: rotate({i * 45}deg); animation-delay: {i * 100}ms"></div>
      {/each}
    </div>
...
<style>
  ...
  @keyframes processing-idle {
    ...
    50% { transform: translate(-50%, -50%) rotateY(-2deg); }
  }
  ...
  .trace-1 { width: 90%; animation-delay: 0s; }
  .trace-2 { width: 70%; animation-delay: 0.5s; }
  .trace-3 { width: 50%; animation-delay: 1s; }
  ...
  .gpu-container.hovered .gpu-body {
    animation: processing-idle 2s ease-in-out infinite, hover-glow 2s ease-in-out infinite;
  }
