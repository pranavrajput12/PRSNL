<script lang="ts">
  ...
  function announceProgress(value: number) {
    ...
    } else if (roundedValue % announceInterval === 0 || 
               roundedValue === 25 || 
               roundedValue === 75) {
    ...
    if (message.type === MessageType.PROGRESS && itemId) {
      try {
        ...
        if (message.data.item_id === itemId) {
          ...
          if (typeof message.data.progress === 'number' && message.data.progress !== progressValue) {
      ...
      <div class="progress-header">
        {#if showIcon}
          <div class="icon-container" class:spin={animated && progress < 100 && !initialLoad}>
            ...
            {:else}
              <Icon name={getStatusIcon()} size="small" color={progress >= 100 ? 'var(--success)' : color} />
            {/if}
          </div>
        {/if}
    ...
    <div class="progress-bar-container">
      ...
      {:else}
        <div class="progress-bar" 
          style="width: {displayedProgress}%; background-color: {displayedProgress >= 100 ? 'var(--success)' : color};"
...
<style>
  ...
  .skeleton-bar {
    width: 60%;
    background: var(--bg-tertiary);
    animation: pulse 1.5s infinite ease-in-out, skeleton-progress 2s infinite ease-in-out;
  }
