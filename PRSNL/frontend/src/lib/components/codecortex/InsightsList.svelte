  ...
  const filteredInsights = $derived(
    insights.filter((insight) => {
      const matchesAnalysis =
        selectedAnalysis === 'all' || insight.analysis_id === selectedAnalysis;
    ...
    try {
      ...
      for (const analysis of completedAnalyses) {
        const response = await api.get(
          `/codemirror/insights/${analysis.id}?status=${selectedStatus}`
        );
...
<div class="insights-container">
  <div class="insights-controls">
    <div class="filter-row">
      <select class="filter-select" bind:value={selectedAnalysis}>
            ...
            {#if insight.severity}
              <span
                ...
                style="background-color: {getSeverityColor(
                  insight.severity
                )}20; color: {getSeverityColor(insight.severity)}"
              >
                {insight.severity}
              </span>
            {/if}
