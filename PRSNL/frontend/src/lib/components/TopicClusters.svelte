<script lang="ts" type="module">
  ...
  function generateOrganicTree() {
    ...
    sortedData.forEach((topic, index) => {
      const { depth, layer, position } = calculateSmartPosition(
        index,
        sortedData.length,
        totalItems
      );
  ...
  function getSignificantTopics(topics: TopicCluster[], totalItems: number) {
    ...
    return topics.filter((topic) => topic.count >= minTopicSize).slice(0, maxTopics);
  }
  ...
  function findBestParent(targetDepth: number, rootNode: TreeNode, allNodes: TreeNode[]) {
    ...
    const parentCandidates = allNodes.filter(
      (node) => node.depth === targetDepth - 1 && node.children.length < 3
    );
  ...
  function drawTrunk() {
    ...
    const gradient = ctx.createLinearGradient(trunkBase.x, trunkBase.y, trunkTop.x, trunkTop.y);
...
<div class="organic-tree" bind:this={container}>
  {#if data && data.length > 0}
    ...
    {#if hoveredNode}
      <div
        class="node-tooltip"
        style="left: {project3D(hoveredNode.x, hoveredNode.y, hoveredNode.z).x +
          20}px; top: {project3D(hoveredNode.x, hoveredNode.y, hoveredNode.z).y - 10}px;"
      >
