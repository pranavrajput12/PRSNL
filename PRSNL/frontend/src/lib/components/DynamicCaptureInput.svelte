<script lang="ts">
  ...
  const INPUT_CONFIGS: Record<string, InputConfig> = {
    ...
    'development': {
      ...
      visibleFields: ['content', 'title', 'programming_language', 'project_category', 'difficulty_level', 'highlight', 'tags'],
      inputType: 'textarea',
      rows: 8
    },
    'document': {
  ...
  const PROGRAMMING_LANGUAGES = [
    ...
    'PHP', 'Ruby', 'Swift', 'Kotlin', 'Dart', 'HTML', 'CSS', 'SQL', 'Shell', 'Other'
  ];
  
  // Project categories for development content
  const PROJECT_CATEGORIES = [
    'Documentation', 'Frontend', 'Backend', 'DevOps', 'Mobile', 'Data Science', 
    'AI/ML', 'Tutorials', 'Code Snippets', 'Project Notes', 'Learning Paths'
  ];
...
<style>
  ...
  .terminal-input, .terminal-textarea, .terminal-select {
    width: 100%;
  ...
  .terminal-input:focus, .terminal-textarea:focus, .terminal-select:focus {
    outline: none;
    border-color: #00ff88;
