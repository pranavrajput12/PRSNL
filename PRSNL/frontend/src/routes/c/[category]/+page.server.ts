...
export const load: PageServerLoad = async ({ params, url, fetch }) => {
	...
	try {
		...
		const response = await fetch(`http://localhost:8000/api/content/category/${category}?${queryParams}`);
		...
		return {
			...
			pagination: data.pagination || { page: 1, totalPages: 1, total: 0, hasNext: false, hasPrev: false },
			filters: {
				sort,
				search,
				page
