...
export const load: PageServerLoad = async ({ params, fetch, url }) => {
	...
	if (category === 'media') {
		try {
			const apiUrl = url.origin.includes('localhost:3004') || url.origin.includes('localhost:3003') 
				? 'http://localhost:8000' 
				: '';
			const videoResolutionResponse = await fetch(`${apiUrl}/api/video-resolution/${category}/${slug}`);
			...
			if (redirectError && typeof redirectError === 'object' && 'status' in redirectError && redirectError.status === 302) {
				throw redirectError;
			}
