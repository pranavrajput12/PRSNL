...
<div class="docs-page">
          ...
          <div class="doc-meta">
            ...
            {#if doc.difficulty_level}
              <span
                ...
                )}20; color: {getDifficultyColor(doc.difficulty_level)}"
              >
                  ...
                  <div class="component-scores">
                    {#if doc.component_scores.semantic}
                      <span class="score-component semantic"
                        >S: {Math.round(doc.component_scores.semantic * 100)}%</span
                      >
                    {/if}
                    {#if doc.component_scores.keyword}
                      <span class="score-component keyword"
                        >K: {Math.round(doc.component_scores.keyword * 100)}%</span
                      >
                    {/if}
                  </div>
                {/if}
  ...
  {#if hasMore && !loading}
    <div class="load-more-section">
      <button class="load-more-btn" on:click={loadMore}> Load More Documentation </button>
    </div>
  {/if}
