<script lang="ts" type="module">
  ...
  $: readingTime =
    item?.metadata?.ai_analysis?.reading_time ||
  ...
  async function generateTranscript() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          data.detail?.includes('Subtitles are disabled') ||
          data.detail?.includes('No transcript available')
        ) {
          transcriptError =
            'This video does not have captions available. Transcript generation is not possible.';
        } else if (
          data.detail?.includes('YouTube is blocking') ||
          data.detail?.includes('too many requests')
        ) {
          transcriptError =
            'YouTube is temporarily blocking transcript requests. Please try again later.';
          ...
          {#if activeTab === 'overview'}
            <div class="overview-tab">
              ...
              {#if hasAIAnalysis && item.metadata.ai_analysis.sentiment}
                <div
                  class="difficulty-card {getDifficultyColor(item.metadata.ai_analysis.sentiment)}"
                >
              ...
              <KnowledgeCard title="Metadata" icon="database">
                <dl class="metadata-list">
                  ...
                  {#if item.metadata?.video_metadata}
                    <dt>Resolution</dt>
                    <dd>
                      {item.metadata.video_metadata.width}x{item.metadata.video_metadata.height}
                    </dd>
