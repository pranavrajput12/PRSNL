...
(async () => {
  try {
        ...
        try {
          ...
          if (route.path === '/videos' || route.path === '/timeline') {
            ...
            const items = await page.$$(
              '[data-testid="content-item"], .content-item, .timeline-item'
            );
      ...
      try {
        ...
        try {
          const title = await page
            .$eval('h1, .title, [data-testid="title"]', (el) => el.textContent)
    ...
    const summary = {
      ...
      avgLoadTime:
        testResults.filter((r) => r.status === 'success').reduce((sum, r) => sum + r.loadTime, 0) /
          testResults.filter((r) => r.status === 'success').length || 0,
      totalConsoleErrors: testResults.reduce(
        (sum, r) => sum + r.consoleMessages.filter((m) => m.includes('[error]')).length,
        0
      ),
