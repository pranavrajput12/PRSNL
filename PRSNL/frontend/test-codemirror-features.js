...
const features = [
  ...
  { name: 'Analysis Type Selection', path: '/code-cortex/codemirror', testId: 'analysis-type-selector' },
  ...
  try {
    ...
    for (const feature of features) {
      ...
      try {
            ...
            switch(feature.testId) {
              ...
              case 'history-section':
                ...
                const items = await page.$$(
                  feature.testId === 'insights-section' ? '.insight-card' : 
                  feature.testId === 'issues-section' ? '.issue-card' : '.history-item'
                );
        ...
        results.push(logResult(feature, featureFound ? 'pass' : 'warn', {
          url,
          screenshot,
        ...
        results.push(logResult(feature, 'fail', {
          error: error.message,
    ...
    try {
      ...
      if (repoLinks.length > 0) {
        ...
        await page.goto(`${CONFIG.baseUrl}${href}`, { waitUntil: 'networkidle2', timeout: CONFIG.waitTimeout });
        const repoDetailScreenshot = await takeScreenshot(page, 'repo-detail-page');
        
        results.push(logResult({ name: 'Repository Detail Page' }, 'pass', {
      ...
      results.push(logResult({ name: 'Repository Detail Page' }, 'fail', {
        error: error.message,
