-- Migration 009: Enhanced Multimodal Embeddings Support
...
-- Create specialized vector indexes for different modalities
CREATE INDEX IF NOT EXISTS idx_embeddings_text_vector ON embeddings USING ivfflat (vector vector_cosine_ops) 
WHERE embedding_type = 'text';

CREATE INDEX IF NOT EXISTS idx_embeddings_image_vector ON embeddings USING ivfflat (vector vector_cosine_ops) 
WHERE embedding_type = 'image';

CREATE INDEX IF NOT EXISTS idx_embeddings_multimodal_vector ON embeddings USING ivfflat (vector vector_cosine_ops) 
...
-- Insert sample embedding types for reference
INSERT INTO public.metadata (key, value) VALUES 
('embedding_types', '["text", "image", "video_frame", "multimodal", "audio"]'::jsonb),
('multimodal_models', '{
    "text": ["text-embedding-ada-002", "text-embedding-3-small"],
    "image": ["clip-vit-base-patch32", "clip-vit-large-patch14"],
    "multimodal": ["clip-vit-base-patch32", "imagebind-large"],
    "audio": ["whisper-embedding", "wav2vec2"]
}'::jsonb)
