...
class TwitterProcessor(PlatformProcessor):
    def __init__(self):
        super().__init__()
    ...
    async def get_info(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Error getting Twitter video info for {url}: {e}")
            return None
    ...
    async def download(self, url: str, output_path: str, progress_hook: Optional[callable] = None) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Error downloading Twitter video from {url}: {e}")
            raise
